...
print(
    f"Without SSH: {no_ssh_time:.3f}s ({no_ssh_time/iterations*1000:.1f}ms per request)")
print(
    f"With SSH: {ssh_time:.3f}s ({ssh_time/iterations*1000:.1f}ms per request)")
...
print(
    f"Without SSH (manual): {no_ssh_time_manual:.3f}s ({no_ssh_time_manual/iterations*1000:.1f}ms per request)")
print(
    f"With SSH (manual): {ssh_time_manual:.3f}s ({ssh_time_manual/iterations*1000:.1f}ms per request)")
print(
    f"Overhead (manual): {((ssh_time_manual/no_ssh_time_manual - 1) * 100):.1f}%")
